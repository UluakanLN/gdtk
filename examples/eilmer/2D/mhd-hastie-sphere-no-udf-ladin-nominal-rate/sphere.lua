...
-- On configuring the gas model, the setGasModel function returns 3 values: the number of species, 
-- the number of nonequilibrium thermal energy modes and a reference to the gas model
-- object. These items may be handy for making calculations within your input script.
...
-- Note that the energy, temperature and thermal conductivity of the transrotational mode is always stored in the scalar entries. 
-- The array entries are used for the additional nonequilibrium modes. What does that mean exactly? 
-- Take for example the two-temperature model popular in the simulation of high-speed Earth re-entry
-- flows. In this model, the translational and rotational energies in the gas mixture are
-- governed by one temperature. This is the first mode, stored in the scalar entries in
-- the table. A second mode for the internal energy lumps together the vibrational and
-- electronic energy of the internal states. These are described by a second temperature
-- and the values are associated with this second temperature are stored as the first entry in the arrays. As such, we end up with single temperature value in the T modes
-- array, a single energy value in the e modes array, and a single thermal conductivity value in the k modes array. As an example, the following code would be used to
-- print the transrotational and vibroelectronic temperatures in the previously described
...
factor = 120 -- note this is not 5grid-5 (?)
...
-- Solver automatically finds the cell that is nearest to the specified coordinates during analyses
